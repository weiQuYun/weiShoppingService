...
//        1.生成唯一的订单ID 此ID 赋予订单详情用于查询具体商品
        ...
        //-----------------------------------------------------------------------------------暂定在这个地方判断
        ShMember moneyMenber = shMemberMapper.selectByid(shCart.getUserId());
        if (moneyMenber.getLvVip() >= 1) {
            Boolean payOk = judgeMoneyForVip(moneyMenber, shOrder.getTotalPrice());
            if (payOk) {
    ...
    private Boolean judgeMoneyForVip(ShMember moneyMenber, BigDecimal totalPrice) {
        ...
        BigDecimal amount = shMoney.getAmount();//用户实际钱
        if (amount.doubleValue() - totalPrice.doubleValue() > 0) {
            return true;
        }
        return false;
    }
    ...
    public Boolean updateShOrder(ShOrder shOrder) {
        ...
        if (shOrder.getPayStatus() == 1) {
            ...
            if (shMember.getLvVip() == 0) {
                ...
                mothMoneyUtils.MothMoneyUtilss(shOrder.getTotalPrice(), shOrder.getMemberId());
                return true;
            }
            //2.购买够10次了 清除所有连锁
